...
import sys
sys.path.insert(1, '/hpc/personal_class/')
import numpy as np
import pandas as pd
import pickle as pk
...
# =============================================================================
# author_dir = '/home/rmi/Projects/Topic_Sentiment/00_data/'
...
# =============================================================================
author = 'MT Newswires'
db_dir = '/hpc/Data/NASDAQ/02_11authors_lemtxt_db/'
...
search_param = {'n_components': [15, 20, 25, 30, 35, 40]}
...
# =============================================================================
out_dir = '/hpctmp/Data/NASDAQ_Related/04_11authors_lda_indiv/'
df_cv_results = pd.DataFrame.from_dict(model.cv_results_)
df_cv_results.to_csv(out_dir + 'p_crossValid_summary_' + 'mtne' + '.csv')
...
with open(out_dir + 'p_cv_bestmodel_mtne.pk', 'wb') as f:
    pk.dump(best_lda_model, f)

with open(out_dir + 'p_vocab_mtne.pk', 'wb') as f2:
    pk.dump(vocab, f2)

with open(out_dir + 'p_docwds_mtne.pk', 'wb') as f3:
    pk.dump(X, f3)

with open(out_dir + 'p_tpcwds_mtne.pk', 'wb') as f4:
    pk.dump(tpc_wds_mat , f4)

with open(out_dir + 'p_doctpc_mtne.pk', 'wb') as f5:
